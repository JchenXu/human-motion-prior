...
def data_prepare(dorig, smpl, vposer_pt, block_size, frequency_num, batch_size=1, frame_num=128, noise_level=180, trans_convert=True, recomp_p3ds=False, device=torch.device('cuda')):
    ...
    std_euler_org[:, 2] = np.random.rand(bs) * 2 * noise_level - noise_level 
    ##################################
    ...
    root_orient_aa_ = MotionPrior.matrot2aa(root_orient_matrot_).reshape(batch_size, frame_num, 3)
    ...
    root_orient_matrot_ = root_orient_matrot_.reshape(batch_size, frame_num, 3,
                                                      3)  # (B, FRAME, 3, 3)
    ...
    root_orient_cont_ = root_orient_cont_.reshape(batch_size, frame_num, -1)  # # (B * F, 6)

    # (B, FRAME_NUM * (latentD + 3 + 6))
    # latent_code_sample = torch.cat([latent_code_sample, root_orient_rotmat, trans], dim=-1).reshape(
    #     batch_size, -1)
    if recomp_p3ds:
        dorig['p3ds'] = get_p3ds(smpl, pose_aa, root_orient_aa_, trans)
    ...
    item = {'pose_aa': pose_aa, 'vpose_seq': vpose_seq,
            'root_orient_aa': root_orient_aa_norm, 'root_orient_aa_norm': root_orient_aa_,
            'root_orient_cont': root_orient_cont_, 'trans': trans,
