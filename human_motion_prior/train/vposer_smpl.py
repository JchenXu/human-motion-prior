...
class Discriminator(nn.Module):
    ...
    def forward(self, Pin):
        ...
        Xout = F.leaky_relu(self.fc1(Xout), negative_slope=.2)
        # Xout = self.bn1(Xout)
        Xout = F.leaky_relu(self.fc2(Xout), negative_slope=.2)
        logits = Xout
        Xout = self.sigmoid_(self.fc3(Xout))

        return logits, Xout

class Encoder(nn.Module):
    def __init__(self, n_features, num_neurons, latentD):
...
class VPoser(nn.Module):
    def __init__(self, num_neurons, latentD, data_shape, use_cont_repr=True):
        ...
        n_features = np.prod(data_shape)
        self.encoder = Encoder(n_features, num_neurons, latentD)
        self.decoder = Decoder(num_neurons, latentD, data_shape, use_cont_repr)
    ...
    def forward(self, Pin, input_type='matrot', output_type='matrot'):
        ...
        q_z = self.encoder(Pin)
        q_z_sample = q_z.rsample()
        Prec = self.decoder(q_z_sample)
...
class VPoserTrainer:
    ...
    def __init__(self, work_dir, ps):
        ...
        from human_motion_prior.data.dataloader import VPoserDS
        ...
        self.discriminator = Discriminator(data_shape=ps.data_shape)
        ...
        self.discriminator.to(self.comp_device)

        E_varlist = [var[1] for var in self.vposer_model.encoder.named_parameters()]
        D_varlist = [var[1] for var in self.vposer_model.decoder.named_parameters()]
        Dis_varlist = [var[1] for var in self.discriminator.named_parameters()]
        ...
        self.E_optimizer = optim.Adam(E_varlist, lr=ps.base_lr, weight_decay=ps.reg_coef)
        self.D_optimizer = optim.Adam(D_varlist, lr=ps.base_lr, weight_decay=ps.reg_coef)
        self.Dis_optimizer = optim.Adam(Dis_varlist, lr=ps.base_lr, weight_decay=ps.reg_coef)
        ...
        self.bm = BodyModel(self.ps.bm_path, batch_size=self.ps.batch_size, use_posedirs=True).to(self.comp_device)
    ...
    def train(self):
        ...
        ones_label = torch.ones(self.ps.batch_size, 1).float().to(self.comp_device)     # one for real sample
        zeros_label = torch.zeros(self.ps.batch_size, 1).float().to(self.comp_device)   # zero for fake
        for it, dorig in enumerate(self.ds_train):
            dorig = {k: dorig[k].to(self.comp_device) for k in dorig.keys()}    # input data

            drec = self.vposer_model(dorig['pose_aa'], output_type='aa')
            logits_org, label_org = self.discriminator(dorig['pose_aa'])
            logits_rec, label_rec = self.discriminator(drec['pose_aa'])
            dis_acc = (label_rec < 0.5).int().sum().item() * 100.0 / ones_label.sum().item()
            ...
            loss_rec_D = ((label_rec - ones_label) ** 2).mean()
            (loss_total + loss_rec_D).backward(retain_graph=True)
            self.D_optimizer.step()

            self.Dis_optimizer.zero_grad()
            loss_rec_Dis = ((label_rec - zeros_label) ** 2).mean()
            loss_org_Dis = ((label_org - ones_label) ** 2).mean()
            (loss_rec_Dis + loss_org_Dis).backward()
            ...
            if it % (save_every_it + 1) == 0:
                ...
                train_msg = VPoserTrainer.creat_loss_message(cur_train_loss_dict, expr_code=self.ps.expr_code,
                                                             ...
                                                             try_num=self.try_num, dis_acc=dis_acc, mode='train')
    ...
    def perform_training(self, num_epochs=None, message=None):
        ...
        prev_lr = np.inf
        E_scheduler = torch.optim.lr_scheduler.StepLR(self.E_optimizer, step_size=int(num_epochs // 3), gamma=0.5)
        D_scheduler = torch.optim.lr_scheduler.StepLR(self.D_optimizer, step_size=int(num_epochs // 3), gamma=0.5)
        Dis_scheduler = torch.optim.lr_scheduler.StepLR(self.Dis_optimizer, step_size=int(num_epochs // 3), gamma=0.5)
        for epoch_num in range(1, num_epochs + 1):
            E_scheduler.step()
            D_scheduler.step()
            Dis_scheduler.step()
            cur_lr = self.E_optimizer.param_groups[0]['lr']
            if cur_lr != prev_lr:
    ...
    @staticmethod
    def creat_loss_message(loss_dict, expr_code='XX', epoch_num=0, it=0, try_num=0, dis_acc=-1, mode='evald'):
        ext_msg = ' | '.join(['%s = %.2e' % (k, v) for k, v in loss_dict.items() if k != 'loss_total'])
        return '[%s]_TR%02d_E%03d - It %05d - %s: [T:%.2e] - [%s] / Dis Acc: [%02d]' % (
        expr_code, try_num, epoch_num, it, mode, loss_dict['loss_total'], ext_msg, dis_acc)
